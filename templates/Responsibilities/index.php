<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\App\Model\Entity\Responsibility> $responsibilities
 */
?>
<div class="responsibilities index content">
    <?= $this->Html->link(__('New Responsibility'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Responsibilities') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('book_id') ?></th>
                    <th><?= $this->Paginator->sort('user_id') ?></th>
                    <th><?= $this->Paginator->sort('action') ?></th>
                    <th><?= $this->Paginator->sort('changes') ?></th>
                    <th><?= $this->Paginator->sort('created') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($responsibilities as $responsibility): ?>
                <tr>
                    <td><?= $this->Number->format($responsibility->id) ?></td>
                    <td><?= $responsibility->hasValue('book') ? $this->Html->link($responsibility->book->original_title, ['controller' => 'Books', 'action' => 'view', $responsibility->book->id]) : '' ?></td>
                    <td><?= $responsibility->hasValue('user') ? $this->Html->link($responsibility->user->name, ['controller' => 'Users', 'action' => 'view', $responsibility->user->id]) : '' ?></td>
                    <td><?= h($responsibility->action) ?></td>
                    <td><?= h($responsibility->changes) ?></td>
                    <td><?= h($responsibility->created) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $responsibility->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $responsibility->id]) ?>
                        <?= $this->Form->postLink(
                            __('Delete'),
                            ['action' => 'delete', $responsibility->id],
                            [
                                'method' => 'delete',
                                'confirm' => __('Are you sure you want to delete # {0}?', $responsibility->id),
                            ]
                        ) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>